...
const EmailVerifictionScreen = ({navigation, route}) => {
  ...
  const {user, isSignUp} = route.params;
  ...
  const handleVerificationCodePress = () => {
    ...
    data.append('pin', codeone);
    setloading(true);
    fetch('https://bicicita.com/app/api/verify', {
      method: 'POST',
      body: data,
      headers: {
      ...
      .then((res) => {
        if (res.status === 'success') {
          if (isSignUp) {
            ToggleLoginSignup(user)(dispatch);
          } else {
...
export default EmailVerifictionScreen;
